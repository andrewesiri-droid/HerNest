...
const Ic={
  ...
  Sunrise: p=><svg width={p.s||22} height={p.s||22} viewBox="0 0 24 24" fill="none"><path d="M12 6V3" stroke={p.c||T.gold} strokeWidth={p.w||1.5} strokeLinecap="round"/><path d="M5.5 12.5L3.5 10.5M18.5 12.5l2-2" stroke={p.c||T.gold} strokeWidth={p.w||1.5} strokeLinecap="round"/><path d="M3 17h18" stroke={p.c||T.gold} strokeWidth={p.w||1.5} strokeLinecap="round"/><path d="M5 17a7 7 0 0114 0" stroke={p.c||T.gold} strokeWidth={p.w||1.5} strokeLinecap="round"/><path d="M8 21h8" stroke={p.c||T.gold} strokeWidth={p.w||1.5} strokeLinecap="round"/></svg>,
  Bowl:    p=><svg width={p.s||22} height={p.s||22} viewBox="0 0 24 24" fill="none"><path d="M4 10h16a8 8 0 01-16 0z" stroke={p.c||T.esp} strokeWidth={p.w||1.5} strokeLinejoin="round"/><path d="M12 2v3M8 3.5l1 2M16 3.5l-1 2" stroke={p.c||T.esp} strokeWidth={p.w||1.5} strokeLinecap="round"/><path d="M8 20h8" stroke={p.c||T.esp} strokeWidth={p.w||1.5} strokeLinecap="round"/></svg>,
  Candle:  p=><svg width={p.s||22} height={p.s||22} viewBox="0 0 24 24" fill="none"><path d="M12 2c0 0-1.5 1.5-1.5 3s1.5 1.5 1.5 3" stroke={p.c||T.gold} strokeWidth={p.w||1.5} strokeLinecap="round"/><rect x="8" y="10" width="8" height="12" rx="2" stroke={p.c||T.gold} strokeWidth={p.w||1.5}/></svg>,
  Plane:   p=><svg width={p.s||22} height={p.s||22} viewBox="0 0 24 24" fill="none"><path d="M21 16l-9-4.5L10.5 3 9 11.5 0 16l8.5-.5L10.5 22l1.5-6L21 16z" stroke={p.c||T.esp} strokeWidth={p.w||1.5} strokeLinejoin="round"/></svg>,
  Globe:   p=><svg width={p.s||22} height={p.s||22} viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="9" stroke={p.c||T.esp} strokeWidth={p.w||1.5}/><path d="M12 3c-2.5 3-4 5.5-4 9s1.5 6 4 9" stroke={p.c||T.esp} strokeWidth={p.w||1.4}/><path d="M12 3c2.5 3 4 5.5 4 9s-1.5 6-4 9" stroke={p.c||T.esp} strokeWidth={p.w||1.4}/><line x1="3" y1="12" x2="21" y2="12" stroke={p.c||T.esp} strokeWidth={p.w||1.4}/></svg>,
  Map:     p=><svg width={p.s||22} height={p.s||22} viewBox="0 0 24 24" fill="none"><path d="M9 20L3 17V4l6 3M9 20l6-3M9 20V7M15 17l6 3V7l-6-3M15 17V4M9 7l6-3" stroke={p.c||T.esp} strokeWidth={p.w||1.5} strokeLinecap="round" strokeLinejoin="round"/></svg>,
...
// ─── SHARED COMPONENTS ───────────────────────────────────────────
function Card({ch,sx={}}){return <div className="lift" style={{background:"#fff",borderRadius:22,padding:"18px",boxShadow:"0 4px 20px rgba(30,20,10,.08)",border:`1px solid ${T.linen}`,marginBottom:12,...sx}}>{ch}</div>;}
function H2({t,sub,light}){return <div style={{marginBottom:14}}><h2 style={{fontFamily:FD,fontWeight:600,fontSize:21,letterSpacing:".01em",color:light?"#fff":T.esp,margin:0,fontStyle:"italic"}}>{t}</h2>{sub&&<p style={{fontFamily:FB,fontSize:12,color:light?"rgba(255,255,255,.45)":T.taupe,margin:"4px 0 0",letterSpacing:".01em"}}>{sub}</p>}</div>;}
function Pill({ch,on,active,color}){return <button onClick={on} style={{flexShrink:0,border:`1.5px solid ${active?(color||T.esp):"transparent"}`,cursor:"pointer",padding:"7px 16px",borderRadius:22,fontFamily:FB,fontSize:12,fontWeight:600,background:active?(color||T.esp):T.sand,color:active?"#fff":T.bark,transition:"all .2s",letterSpacing:".01em"}}>{ch}</button>;}
...
// ═══════════════════════════════════════════════════════════════════
function SchoolCalendar({profile,uid}){
...
5 priorities, 3-4 reminders, include energyTip and a one-word focusWord for the day (e.g. "Focus", "Rest", "Connect").`;
    const ctx=`Name: ${profile?.name||"Sarah"}, role: ${profile?.role||"CFO"}, ${familyCtx}, trip: ${profile?.tripGoal||"none"}, fitness: ${profile?.fitnessGoal||"none"}, challenge: ${profile?.challenge||"mental load"}, priorities: ${profile?.priorities?.join(",")||"family,career,fitness"}. ${bdayCtx} ${calCtx} ${schoolCtx}`;
...
export default function HerNest(){
  ...
  useEffect(()=>{
    if(user?.uid&&profile.name){
      saveProfile(user.uid,profile).catch(e=>console.log("Save error:",e));
    }
  },[profile,user]);
    ...
    return(
      <div style={{maxWidth:430,margin:"0 auto",minHeight:"100vh",background:T.cream,boxShadow:"0 0 80px rgba(0,0,0,.3)"}}>
        ...
        <div style={{padding:"16px 24px 40px"}}>
          ...
          {screen==="step4"&&<Step4 data={profile} onChange={upd} onFinish={()=>{if(user?.uid)saveProfile(user.uid,profile);setScreen("step5");}} onBack={()=>setScreen("step3")}/>}
          {screen==="step5"&&<Step5 data={profile} onChange={upd} onNext={()=>setScreen("step6")} onBack={()=>setScreen("step4")}/>}
          {screen==="step6"&&<Step6 data={profile} onChange={upd} onFinish={()=>{if(user?.uid)saveProfile(user.uid,profile);setScreen("intro");}} onBack={()=>setScreen("step5")}/>}
        </div>
      </div>
    );
  }
