...
const getWeeklyMood=()=>{try{return JSON.parse(localStorage.getItem("hn_weekly_mood")||"null");}catch(e){return null;}};
const submitWeeklyCheckIn=async(value,uid,saveData)=>{const mood={weekStart:getWeekStart(),value,label:MOOD_LEVELS.find(m=>m.value===value)?.label};try{localStorage.setItem("hn_weekly_mood",JSON.stringify(mood));}catch(e){}if(uid)await saveData(uid,"wellness",{weeklyMood:mood}).catch(()=>{});};
